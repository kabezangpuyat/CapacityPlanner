...
namespace WFMPCP.Service
{
	public class RoleService : IRoleService
	{
		...
		public RoleService(IWFMPCPContext context)
		{
		...
		{
			var data = _context.AsQueryable<Role>()
		...
		{
			var data = _context.AsQueryable<Role>().Where(x => x.ID == id).FirstOrDefault();
			if (data == null)
			...
			_context.Update<Role>(data);
			_context.SaveChanges();
		}

		public void Update(RoleViewModel model)
		{
			var data = _context.AsQueryable<Role>().Where(x => x.ID == model.ID).FirstOrDefault();
			if (data == null)
				throw new ArgumentException("Data does not exists.");
			...
			{
				var dataExists = _context.AsQueryable<Role>().Where(x => x.Name == model.Name).FirstOrDefault();
				if (dataExists != null)
