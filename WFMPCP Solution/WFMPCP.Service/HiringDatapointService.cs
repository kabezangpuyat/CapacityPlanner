...
{
	public class HiringDatapointService : IHiringDatapointService
	{
		...
		public IQueryable<HiringDatapointViewModel> GetAll()
		{
			var data = _context.AsQueryable<Data.Entities.HiringDatapoint>()
				.Select(x => new HiringDatapointViewModel()
				{
					ID = x.ID,
		...
		public IQueryable<HiringDatapointViewModel> GetAll(bool? active, bool? visible)
		{
			return this.GetAll().Where(x => ((x.Active == active) || (active == null)) && ((x.Visible == visible) || (visible == null))).AsQueryable();
		}

		public HiringDatapointViewModel GetByID(long id)
		{
			return this.GetByID(id, null, null);
		}

		public HiringDatapointViewModel GetByID(long id, bool? active, bool? visible)
		{
		...
		{
			var data = _context.AsQueryable<Data.Entities.HiringDatapoint>().Where(x => x.ID == id).FirstOrDefault();
			if (data == null)
