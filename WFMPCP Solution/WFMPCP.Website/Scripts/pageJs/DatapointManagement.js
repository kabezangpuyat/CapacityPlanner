var datapointmanagement = {
    OnSaveClick: function (saveURL, gridTable) {
        $('#btnSave').click(function () {
            ...
            if (siteID == '0')
                msg += 'Segment is required. <br />';

            if (msg != '') {
                $.alert({
                    closeIcon: true,
                    title: 'WFMPCP | Datapoint Manager',
                    content: msg
                });
            }
            else {
                ...
                $.confirm({
                    ...
                    confirm: function () {
                        ...
                        $.post(saveURL, model, function (data) {
                            ...
                            datapointmanagement.ClearData(gridTable);
                        }, 'json');
                    }
                });
            }
    ...
    OnGridSelectRow: function (id, gridSelectUrl) {
        $.post(gridSelectUrl, { "id": id }, function (data) {
            ...
            $('#ddlCategories').val(data.SegmentID);
            $('#ddlDatatypes').val(data.Datatype);
            //alert(data.Datatype)
            var active = '1';
